...
export function useUserSettings(userAddress: Address | null) {
  ...
  const updateSettings = useCallback(
    async (newSettings: Partial<UserSettings>): Promise<boolean> => {
      ...
      try {
        ...
        if (upsertError) {
          console.error("[useUserSettings] Upsert error:", upsertError);
  ...
  const setStatus = useCallback(
    (emoji: string, text: string) => updateSettings({ statusEmoji: emoji, statusText: text }),
    [updateSettings]
  );
  ...
  const clearStatus = useCallback(
    () => updateSettings({ statusEmoji: "💬", statusText: "Available to chat" }),
    [updateSettings]
  );
...
export function useFriendStatus(friendAddress: Address | null) {
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      const { data } = await client
        ...
        .eq("wallet_address", friendAddress.toLowerCase())
        .maybeSingle();
    ...
    const channel = client
      ...
      .on(
        "postgres_changes",
        {
          ...
          filter: `wallet_address=eq.${friendAddress.toLowerCase()}`,
        },
        (payload) => {
