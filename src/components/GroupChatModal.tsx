...
interface GroupChatModalProps {
  ...
  friends?: Friend[];
  onGroupDeleted?: () => void;
  onStartCall?: (groupId: string, groupName: string, isVideo: boolean) => void;
...
export function GroupChatModal({
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        // Start streaming
        const stream = await streamGroupMessages(group.id, (message: unknown) => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  }, [isOpen, isInitialized, group, getGroupMessages, getGroupMembers, streamGroupMessages, markGroupAsRead]);
  ...
  const handleSendPixelArt = useCallback(async (imageData: string) => {
    ...
    try {
      const uploadResponse = await fetch("/api/pixel-art/upload", {
        ...
        body: JSON.stringify({ imageData, senderAddress: userAddress }),
      });
      ...
      const result = await sendGroupMessage(group.id, pixelArtMessage);
      ...
      setError(`Failed to send pixel art: ${err instanceof Error ? err.message : "Unknown error"}`);
    } finally {
  ...
  const availableFriends = friends.filter((friend) => {
    ...
    return !members.some((m) =>
      m.addresses.some((addr) => addr.toLowerCase() === friendAddressLower)
    );
  });
  ...
  const getDisplayName = (friend: Friend) => {
    return friend.nickname || friend.shoutUsername || friend.ensName || 
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl h-full flex flex-col overflow-hidden">
                  ...
                  >
                    <div className="p-3 bg-zinc-800/50 max-h-40 overflow-y-auto">
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-zinc-500 text-xs">Group Members ({members.length})</p>
                        <button
                      ...
                      <div className="space-y-1">
                        {members.map((member) => {
                          const isMe = member.inboxId === userInboxId;
                          ...
                          return (
                            <div
                              ...
                              className={`flex items-center justify-between px-2 py-1.5 rounded-lg ${
                                isMe ? "bg-violet-500/20" : "bg-zinc-700/50"
                              }`}
                            >
                              <span className={`text-xs ${isMe ? "text-violet-300" : "text-zinc-300"}`}>
                                {isMe ? "You" : formatAddress(memberAddress)}
                              </span>
                              {!isMe && (
                                <button
                                  onClick={() => handleRemoveMember(memberAddress)}
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {isLoading ? (
                  ...
                  messages.map((msg) => {
                    const isOwn = msg.senderInboxId === userInboxId;
                        ...
                        >
                          {!isOwn && (
                            <p className="text-xs text-zinc-400 mb-1">
                              {members.find(m => m.inboxId === msg.senderInboxId)?.addresses[0]
                                ? formatAddress(members.find(m => m.inboxId === msg.senderInboxId)!.addresses[0])
                                : "Unknown"}
                            </p>
                          )}
                          {isPixelArt ? (
                          ...
                          <p className={`text-xs mt-1 ${isOwn ? "text-violet-200" : "text-zinc-500"}`}>
              ...
              <div className="p-4 border-t border-zinc-800">
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={handleSend}
                    disabled={!newMessage.trim() || isSending || !isInitialized}
                  ...
                  {availableFriends.length === 0 ? (
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                      {availableFriends.map((friend) => (
                        <button
                          key={friend.id}
                          onClick={() => handleAddMember(friend.address)}
                        ...
                        >
                          {friend.avatar ? (
                            ...
                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center text-white text-xs font-bold">
                              {getDisplayName(friend).slice(0, 2).toUpperCase()}
                            </div>
                          )}
